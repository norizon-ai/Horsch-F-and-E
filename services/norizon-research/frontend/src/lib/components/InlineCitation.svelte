<script lang="ts">
	...
	$: iconType = getSourceIcon(source.sourceType);
...
<span
	...
	on:click={() => source.url && window.open(source.url, "_blank")}
	on:keypress={(e) =>
		e.key === "Enter" && source.url && window.open(source.url, "_blank")}
>
	<Badge
		variant="secondary"
		class="inline-flex items-center justify-center w-[18px] h-[18px] !rounded bg-nora-blue-100 text-nora-blue-600 text-[11px] font-semibold cursor-pointer !p-0 !border-0 hover:bg-nora-blue-500 hover:text-white transition-all duration-150"
	>
		{index}
	</Badge>
	{#if showTooltip}
