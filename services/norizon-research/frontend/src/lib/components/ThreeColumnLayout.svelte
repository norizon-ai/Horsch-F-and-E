...
<div class="h-screen flex flex-col bg-transparent">
	...
	<div class="flex-1 flex overflow-hidden relative">
		...
		<aside
			class="hidden lg:flex lg:flex-col lg:w-72 bg-white/80 backdrop-blur-md border-r border-gray-200/50 overflow-y-auto"
		>
			<div class="p-4">
		...
		{#if $showSources}
			<aside
				class="hidden lg:block lg:w-80 xl:w-96 bg-white border-l border-gray-200 overflow-y-auto"
			>
		...
		<!-- Right Panel: Mobile Drawer (Sources) -->
		{#if rightDrawerOpen && isMobile}
			<div class="fixed inset-0 z-50">
				<div
					class="absolute inset-0 bg-black/50"
					on:click={() => (rightDrawerOpen = false)}
				></div>
				<aside
					class="absolute right-0 top-0 bottom-0 w-full bg-white shadow-xl overflow-y-auto"
				>
					<div class="p-4 border-b border-gray-200">
						<div class="flex items-center justify-between mb-4">
							<h2
	...
	{#if isMobile}
		<nav
			class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-40 safe-area-inset-bottom"
		>
			<div class="flex justify-around items-center h-16">
				...
				<button
					...
					class:text-norizon-blue={leftDrawerOpen}
				...
				<button
					...
					class="flex flex-col items-center justify-center flex-1 h-full hover:bg-gray-50 active:bg-gray-100 transition-colors"
					class:text-norizon-orange={rightDrawerOpen}
