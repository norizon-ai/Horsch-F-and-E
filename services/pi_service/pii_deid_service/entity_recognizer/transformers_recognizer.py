...
# based on https://github.com/microsoft/presidio/blob/main/docs/samples/python/transformers_recognizer/transformer_recognizer.py
...
class TransformersRecognizer(EntityRecognizer):
    ...
    def __init__(
        ...
        if not model_config:
            raise Exception("no model config provided")

        self.model_config = model_config

        self.supported_language = model_config.get("SUPPORTED_LANGUAGE", "en")
        self.supported_entities = model_config.get("PRESIDIO_SUPPORTED_ENTITIES", [])
        ...
        super().__init__(
            ...
            name=f"Transformer model {model_path}",
            supported_language=self.supported_language
            # Change this to 'de' to use model in german pipeline; Downside: one cannot simply mix a german transformer recognizer with the default en pattern recognizers.
        )

    def load(self) -> None:
        """Load external configuration parameters and set default values.

        :param model_config: define default values for class attributes and modify pipeline behavior
        ...
        **ID_SCORE_REDUCTION (float) - score multiplier for ID entities
        """
        self.entity_mapping = self.model_config.get("DATASET_TO_PRESIDIO_MAPPING", {})
        self.model_to_presidio_mapping = self.model_config.get("MODEL_TO_PRESIDIO_MAPPING", {})
        self.ignore_labels = self.model_config.get("LABELS_TO_IGNORE", ["O"])
        self.aggregation_mechanism = self.model_config.get("SUB_WORD_AGGREGATION", "simple")
        self.default_explanation = self.model_config.get("DEFAULT_EXPLANATION", None)
        self.text_overlap_length = self.model_config.get("CHUNK_OVERLAP_SIZE", 40)
        self.chunk_length = self.model_config.get("CHUNK_SIZE", 600)
        self.id_entity_name = self.model_config.get("ID_ENTITY_NAME", "ID")
        self.id_score_reduction = self.model_config.get("ID_SCORE_REDUCTION", 0.5)
    ...
    def _load_pipeline(self) -> None:
        ...
        logging.info(f"Initializing NER pipeline using {self.model_path} path")
    ...
    def analyze(
        ...
        :param entities: Entities to be detected
    ...
    def __check_label_transformer(self, label: str) -> Optional[str]:
        ...
        if entity is None:
            logging.warning(f"Found unrecognized label {label}, returning entity as is")
            return label

        if entity not in self.supported_entities:
            logging.warning(f"Found entity {entity} which is not supported by Presidio")
            return entity
        return entity