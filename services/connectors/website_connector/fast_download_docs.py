...
"""
ONLY A REFERENCE FILE - NOT USED IN PRODUCTION
"""
...
class FastDocScraper:
    def __init__(
        self, base_url: str = "https://doc.nhr.fau.de", output_dir: str = "docsmd"
    ):
    ...
    def download_with_wget(self):
        ...
        wget_cmd = [
            ...
            "--domains",
            "fau.de",  # Stay within domain
            "--no-parent",  # Don't go up in directory structure
            "--reject",
            "*.pdf,*.zip,*.tar.gz,*.exe,*.dmg,*.jpg,*.jpeg,*.png,*.gif,*.svg,*.ico,*.webp,*.bmp,*.tiff,*.css,*.js",  # Skip binary files and images
            "--directory-prefix",
            str(self.html_dir),
            "--user-agent",
    ...
    def html_file_to_url(self, html_file: Path) -> str:
        ...
        domain_dir = self.html_dir / "fau.de"
    ...
    def process_all_html_files(self):
        ...
        logger.info(
            f"Processing completed. Converted {processed_count} HTML files to markdown."
        )
...
def main():
    ...
    scraper = FastDocScraper(base_url="https://www.fau.de/", output_dir="fau_docsmd")
