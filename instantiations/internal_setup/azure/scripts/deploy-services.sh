...
for SERVICE_NAME in $DEPLOY_SERVICES; do
    ...
    if [ "$SERVICE_NAME" = "frontend" ]; then
        BUILD_ARGS="--build-arg VITE_WORKFLOW_API_URL=${VITE_WORKFLOW_API_URL:-}"
        [ -n "${VITE_DEEPSEARCH_API_URL:-}" ] && BUILD_ARGS="$BUILD_ARGS --build-arg VITE_DEEPSEARCH_API_URL=$VITE_DEEPSEARCH_API_URL"
        # SSR: server-side calls use ACA internal DNS (container-to-container)
        BUILD_ARGS="$BUILD_ARGS --build-arg VITE_SSR_API_BASE_URL=http://deepsearch:8000/api/v1"
    fi

    docker build --platform linux/amd64 -t "$FULL_IMAGE" -t "$LATEST_IMAGE" $BUILD_ARGS "$BUILD_CONTEXT"
...
for SERVICE_NAME in $DEPLOY_SERVICES; do
    echo "  Updating $SERVICE_NAME → norizon/$SERVICE_NAME:$IMAGE_TAG"
    ...
    az containerapp update \
        --name "$SERVICE_NAME" \
        --resource-group "$AZURE_RESOURCE_GROUP" \
        --image "$ACR_SERVER/norizon/$SERVICE_NAME:$IMAGE_TAG" \
        --output none &
done
